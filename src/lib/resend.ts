    ...
    <body style="font-family: Inter, system-ui, sans-serif; line-height: 1.6; color: #1a1a2e; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; padding: 24px; margin-bottom: 24px;">
        ...
        <ul style="list-style: none; padding: 0; margin: 0; font-size: 14px; color: #555;">
          <li style="padding: 4px 0;">Pool size: ${poolSizeLabels[data.approxSize] ?? data.approxSize}</li>
          <li style="padding: 4px 0;">Pool type: ${poolTypeLabels[data.poolType] ?? data.poolType}</li>
          <li style="padding: 4px 0;">Frequency: ${frequencyLabels[data.frequency] ?? data.frequency}</li>
          <li style="padding: 4px 0;">Postcode: ${data.postcode}</li>
          ${data.notes ? `<li style="padding: 4px 0;">Notes: ${data.notes}</li>` : ''}
        </ul>
      </div>
